...
import { Check, LockKeyhole, RefreshCw, Search } from 'lucide-react'
...
export function Admin() {
  ...
  const normalizePlayer = (player: AdminPlayer): AdminPlayer => ({ ...player, status: (player.status as string) === 'fill' || (player.status as string) === 'bench' ? 'fill' : 'roster', assigned_rank: player.assigned_rank ?? 1 })
  const unlock = async () => { setLoading(true); setMessage(''); try { const roster = await rosterApi.adminRoster(secret); setPlayers(roster.map(normalizePlayer)); setUnlocked(true) } catch (error) { setMessage(error instanceof Error ? error.message : 'Access failed.') } setLoading(false) }
  const refresh = async () => { setLoading(true); setMessage(''); try { const roster = await rosterApi.adminRoster(secret); setPlayers(roster.map(normalizePlayer)); setMessage(`Roster refreshed · ${roster.length} players`) } catch (error) { setMessage(error instanceof Error ? error.message : 'Refresh failed.') } setLoading(false) }
  ...
  return <section className="space-y-5"><div className="panel gold-border rounded-2xl p-5 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="rune text-xs font-bold text-red-300">Private planning</p><h2 className="mt-2 text-3xl font-black">Raid leader overview</h2></div><button type="button" onClick={refresh} disabled={loading} className="flex items-center gap-2 rounded-lg border border-stone-700 bg-stone-950 px-3 py-2 text-sm font-bold text-stone-300 transition hover:border-stone-500 hover:text-stone-100 disabled:opacity-50"><RefreshCw size={15} className={loading ? 'animate-spin' : ''} />{loading ? 'Refreshing…' : 'Refresh roster'}</button></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-6"><Summary label="Roster" value={players.filter(player => player.status === 'roster').length} /><Summary label="Fill" value={players.filter(player => player.status === 'fill').length} />{roleCounts.map(item => <Summary key={item.role} label={item.role} value={item.count} />)}</div>
